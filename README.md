...
成员信息请优先由本人提交。代为提交时，需要先确认对方同意公开哪些字段。不要提交手机号、微信号等敏感联系方式。公开联系方式支持 GitHub、个人主页、Google Scholar、ORCID 和经过处理的邮箱。

如果两个成员填写了相同 slug，但姓名不同，系统不会覆盖已有页面，会自动生成 `slug-2`、`slug-3` 这样的新页面。

成员个人页正文可以通过 **我在做什么**、**可交流主题**、**个人经验** 这些字段生成；如果填写 **个人页正文**，则会直接使用这段 Markdown 覆盖自动正文。更新成员时，留空字段会保留原成员页内容。
