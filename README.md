...
成员信息请优先由本人提交。代为提交时，需要先确认对方同意公开哪些字段。公开联系方式支持 GitHub、个人主页、Google Scholar、ORCID、Substack、WeChat 和经过处理的邮箱；手机号、微信号等敏感联系方式仅限本人明确同意公开时填写。

如果两个成员填写了相同 slug，但姓名不同，系统不会覆盖已有页面，会自动生成 `slug-2`、`slug-3` 这样的新页面。

成员个人页正文可以通过 **个人页展示的研究方向**、**我在做什么**、**可交流主题**、**个人经验** 这些字段生成；如果填写 **个人页正文**，则会直接使用这段 Markdown 覆盖自动正文。成员列表和个人页顶部绿色标签来自 `topics`，不会展示“个人页展示的研究方向”。更新成员时，留空字段会保留原成员页内容。
...
如果后续要修改已经发布的经验贴，请编辑原 Issue。系统会根据 `issue_number` 更新原文章文件，不会新建重复文章。
...
如果维护者直接新增、修改或删除 `_members/*.md`，`.github/workflows/member-template-sync.yml` 会自动刷新成员 Issue 表单里的已有 slug 下拉选项。
